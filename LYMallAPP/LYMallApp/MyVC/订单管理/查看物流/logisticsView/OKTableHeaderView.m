...
@property (nonatomic,strong) WKWebView *phoneCallWebView;
@end
...
- (void)setupUI {
    ...
    self.goodsPic.frame=CGRectMake(15, 15, 60,60);
    self.goodsPic.image = [UIImage imageNamed:@"jft_icon_logistics"];
    [self addSubview:self.goodsPic];
    
    self.type =[[UILabel alloc]initWithFrame:CGRectMake(CGRectGetMaxX(self.goodsPic.frame)+14,10, 20*Gap, 2*Gap)];
    self.type.font = [UIFont systemFontOfSize:15];
    [self addSubview:self.type];
    
    
    self.numLabel = [[UILabel alloc]initWithFrame:CGRectMake(CGRectGetMaxX(self.goodsPic.frame)+14 ,25, 230, 2*Gap)];
...
- (void)callPhoneThree:(NSString *)phoneNum{
    ...
    if ( !_phoneCallWebView ) {
        ...
        _phoneCallWebView = [[WKWebView alloc] initWithFrame:CGRectZero];// 这个webView只是一个后台的容易 不需要add到页面上来  效果跟方法二一样 但是这个方法是合法的
    }
